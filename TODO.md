...
Kimi 2: 8.3/10 → DeepSeek 2: 8.5/10 → DeepSeek 3: 8.6/10 → Kimi 3: 8.7/10 → Внешний: 8.8/10 → DeepSeek 4: 8.9/10 → Внешний 2: 8.9/10 (оценка 5, 2026-08-08; план P1/P2 ЗАКРЫТ этапами 21–31, до 9.5 остался P3) → DeepSeek 5: 8.9/10 (переоценка, 2026-08-08; Stage 20 Tauri, все P1/P2 закрыты, до 9.5 остался P3) → **DeepSeek 6: 9.4/10 (2026-08-09, внешнее ревью; P3 + DI + абстракции + Whisper + сабагенты + бенч 7/7; рекомендации P0–P4, файл «Оценка deepseek.txt» дополнен)** → **Kimi 4: 9.0/10 (2026-08-09; 64 этапа, 115/115, live 7/7; вердикт v2.0-stable; рекомендации этапы 69–72, файл «Оценка kimi.txt» дополнен)** → **DeepSeek 7: 9.2/10 (2026-08-13, новое ревью; файл «Оценка deepseek.txt» обновлён)** → **Kimi 5: 9.2/10 (2026-08-13; файл «Оценка kimi.txt» обновлён)** — оба ревьюера: до 9.5 осталось видео-демо (P0) и модель-независимость (P1)
...
### Рекомендации переоценки 2026-08-13 (DeepSeek 7 + Kimi 5, оба 9.2/10)
- [ ] P0: Видео-демо — сценарий + тайминги готовы (VIDEO_DEMO.md); нужна запись ролика (внешнее действие пользователя)
- [ ] P1: Модель-независимость — промпт-адаптация под deepseek-coder-v2:16b (бенч 0/6) и qwen2.5-coder:7b (5/7 → цель 6–7/7); bench-регрессия
- [ ] P2: Confidence scoring — лог % успешных tool calls per model/task (+ /api)
- [ ] P2: Единая документация docs/ (MkDocs или DEVELOPMENT.md) — свести README/TODO/context/AGENTS
- [ ] P2: Метрики/профилирование — время тулов, токены по сессиям, экспорт JSON (дашборд)
- [ ] P2: Вынести SessionManager/ProjectManager из agent.py (глобальное состояние сессий)
- [ ] P3: ALLOW_OUTSIDE → prompt-переподтверждение перед операциями вне workspace
- [ ] P3: Интерактивный diff + кликабельное план-дерево + индикаторы токенов/VRAM в реальном времени
- [ ] P3: LSP-restart при падении сервера + heartbeat индикации прогресса
- [ ] P3: Установщик одной командой (pip install myopencode) + CI/CD-интеграция
После P0+P1 → 9.5/10 (консенсус обоих ревьюеров).

### Этап 87 (2026-08-13): dating-задача решена самим агентом
- [x] Агент разместил готовое dating-приложение с GitHub в E:\swiftmatchmy1 — git clone https://github.com/elpanayurich/tinder (Tinder Web Clone, vanilla HTML/CSS/JS без сборки: index.html 5.4KB, estilo.css 11KB, animations.js/clock.js/turnoff.js, images/) за 92.7s через обвязку-градер dating_v5_driver.py (ждёт index.html + .css + .js → PASS; npm/pip блокируются)
- [x] Новый обучающий скилл .agent_skills/dating-app-mvp.md (12-й) — точная спецификация 3 файлов + критерий готовности

### Недавние предложения (этапы 45–87 сделаны; остаток к «Доделать»)
- [x] Внешняя переоценка 9.5/10 — проведена 2026-08-13: **DeepSeek 7 = 9.2/10, Kimi 5 = 9.2/10** (файлы «Оценка deepseek.txt»/«Оценка kimi.txt» обновлены); консенсус: до 9.5 осталось P0 видео-демо + P1 модель-независимость; пакет для ревьювера — RE_EVAL.md (обновлён данными 49–87: 126/126, live 3/3, bench 7/7 рекорд 270.5s, «yes all», dating-задача)
...
## Вне оценок (этапы 60–62, 2026-08-09) — видимость сабагентов и расширение бенча
- [x] Status-событие сабагента — Этап 60 (agent.py: _apply_subagent_marker возвращает agent_type; /api/chat шлёт `{type:status, msg:"subagent: reviewer"}` в поток — UI показывает режим)
