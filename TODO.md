...
### Критическое
- [x] **Лимит контекста 32K токенов** — исправлено с 8192 → 32768
- [x] **Таймаут агента** — 60с лимит на весь agent loop
- [x] **Verify с пробелами** — пути экранируются кавычками

### Важное
- [ ] **Multi-turn сессии** — сейчас stateless HTTP, после ответа история теряется
- [x] **Diff view в UI** — подсветка ```diff блоков (+/-)
- [x] **Retry JSON** — до 3 попыток парсинга, очистка комментариев
...
- [x] **Отмена операции** — кнопка Cancel в UI
